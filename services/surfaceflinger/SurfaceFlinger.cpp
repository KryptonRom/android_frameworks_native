...
status_t SurfaceFlinger::getDisplayInfo(const sp<IBinder>& display, DisplayInfo* info) {
    ...
    public:
		...
		static int getOverrideDensity()  {
            return getDensityFromProperty("persist.sys.density"); }
    };
    ...
    if (type == DisplayDevice::DISPLAY_PRIMARY) {
		...
		int emudensity = Density::getEmuDensity();
        if (emudensity) {
            // if "qemu.sf.lcd_density" is specified, it overrides ro.sf.lcd_density
            xdpi = ydpi = density = emudensity;
            density /= 160.0f;
        }
		int overridedensity = Density::getOverrideDensity();
        if (overridedensity) {
            // if "persist.sys.density" is specified, it jumps every other motherfucker
            xdpi = ydpi = density = overridedensity;
            density /= 160.0f;
        }
